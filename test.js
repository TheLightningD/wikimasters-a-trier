...
const server = http.createServer((request, response) => {
  ...
  if (pathname === '/sheet') {
    response.end(`google.visualization.Query.setResponse(${JSON.stringify({
      status: 'ok',
      table: { rows: [
        { c: [{ v: null }, { v: 'Pseudo' }, { v: null }, { v: 'Canard' }] },
        { c: [{ v: null }, { v: 'Trucs recherchés' }, { v: 'Nature vivante' }, { v: 'Canard' }] }
      ] }
    })});`);
    return;
  }
...
server.listen(0, '127.0.0.1', async () => {
  ...
  const inventory = await run({
    ...
    WM_COLLECTION_URL: `http://127.0.0.1:${port}/collection.html?all-labeled=1`
  });
  const limited = await run({ SCAN_COLLECTION: 'false', WM_URL: `http://127.0.0.1:${port}/test.html?limit=1` });
  const brokenPull = await run({ SCAN_COLLECTION: 'false', WM_URL: `http://127.0.0.1:${port}/test.html?pull-error=1` });
